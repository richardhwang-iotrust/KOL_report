...
4. Slack #info-telegram_test 채널에 한국어 보고서를 게시하고,
   ...
   ※ 가능하면 한국어·영어 보고서 각각을 5,000자 이내 한 메시지로 맞춘다.
   분량이 많은 날은 항목 설명을 더 간결히 압축하거나(단, 배경 이해에 필요한
   핵심 정보는 유지), 대표 링크 수를 줄이는 방식으로 우선 압축을 시도한다.
   압축해도 5,000자를 넘으면 그때만 섹션 단위(🔥/🔁/📣/👀)로 잘라 본문 +
   스레드 댓글로 이어서 게시한다(한국어 앞부분 섹션은 본문에, 나머지 섹션은
   같은 스레드 댓글로; 영어판도 동일하게 섹션 단위로 나눠 그 뒤를 잇는 댓글로).
